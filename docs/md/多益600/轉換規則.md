...
轉換規則（第一種純文字題目 → 第二種：Markdown + TTS + 正確答案標記 + 克漏字回填）v2
...
- 輸出：## {start}-{end}
- 後接 1 空行
- range-intro 的那一行（"Questions X to Y refer to ..."）屬於內容行：視為一般敘述行（4.4）輸出 TTS。
...
- 先輸出 stem_lines（套用 TTS + 克漏字回填 + 可選 HIGHLIGHT）
- 再輸出 option_lines（套用 TTS + 正確選項加星）
...
5.2 對話行（Dialogue）
- 僅在「對話題」語境內啟用（例如題組導語之後出現多行 speaker 對話）。
- Speaker 白名單（最小集）：W, M
- Regex：^\s*([WM])\s*:\s*(.+?)\s*$

輸出：
- {Speaker}: <span class="tts">{Utterance}</span>

5.3 一般敘述行（Narration / Passage / Question stem）
條件：非空行，且不符合 5.1/5.2。

輸出：
- <span class="tts">{LineTextAfterTransforms}</span>
...
6.1 適用範圍
- 僅對「題幹/敘述行」（5.3）與「對話內容」（5.2 的 Utterance）套用（若你需要）。
- 不對選項文字做克漏字回填。

6.2 回填規則
- 在待輸出的文字中偵測 CLOZE_TOKEN_REGEX（/-{2,}/）。
- 若某一行偵測到 1 個克漏字 token，且「可判定正解」成立：
  - 令 correct_letter = ANSWER_KEY[current_qid]
  - 令 correct_text = option_map[correct_letter]
...
- 若 HIGHLIGHT_MAP[current_qid] 存在，對每個片語 phrase：
  - 在題幹/敘述行（與必要時的對話內容）中，將 phrase 包成 ^^phrase^^。

7.3 保守規則
- 若 phrase 已在某段 ^^...^^ 之內：建議不重複包（避免巢狀）。
  （是否實作可由你轉換器決定；規格建議採保守。）
...
8.2 每個子題標頭（###）後保留 1 空行
8.3 題幹/段落與選項群之間可保留 1 空行（建議固定為：題幹輸出完 → 空行 → 選項群）
...
- 克漏字：All tenants ------ to pay ...（可判定正解且該行只有 1 個 token）→
  All tenants ^^<正確選項文字>^^ to pay ...（再包進 TTS span）
