...
「葉」，但只有一個點的情況，根也是葉。      
3. 子樹（subtree）：在有根樹中，通常指選定某個節點後，由該節點本身與它所有後代節點所組成的樹。若移除某個節點，原樹可能會被分成數個連通分量，這些分量也各自是樹；其中遠離原本根節點方向的那些分量，常可視為該節點各個孩子的子樹。      
4. 父、子關係（parent,child）：有根樹中，如果vi,vj 相鄰，且vi 比較接近根，稱vi 是vj 的父親、vj 是vi的兒子。      
