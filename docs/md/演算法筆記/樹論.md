...
2. 由下往上計算(bottom-up)         
[a078: P_3_1 樹的高度與根 (bottom-up) (APCS201710)](#例題-p-3-1-樹的高度與根-bottom-up-apcs201710-同-p-8-4-1)       
3. 根節點比大小         
[a131: P_8_5 自動分裝 (APCS202002)](#p-8-5-自動分裝-apcs202002)       
4. 陣列傳遞，計算極值         
...
![alt text](images/樹論/image-7.png)

#### 中序

![alt text](<images/樹論/ChatGPT Image 2026年7月28日 下午06_54_35.png>)

#### 後序


![alt text](<images/樹論/ChatGPT Image 2026年7月28日 下午06_57_30.png>)
...
![alt text](../images/樹論/image-1-1.png)
...
![alt text](../images/樹論/SkEL6_XNR.png)
...
int main() {
    ...
    cout << *max_element(d_w.begin(), d_w.end())
         << "\n"
         << *max_element(d_1.begin(), d_1.end());
...
這題會錯很有可能是沒有把 d_w_mi 設為 LLONG_MAX
...
void dfs2(int u, int parent) {
    for (pii i : g[u]) {
        ...
        sum[v] = sum[u] - sub[v] * w + (n - sub[v]) * w;

        // printf("v = %d,sum[v] = %d, sum[u] = %d, w = %d   \n", v, sum[v], sum[u], w);
...
signed main() {
    ...
    for (int i = 0; i < n; i++) {
        if (sum[i] < sum_ma) {
            ...
            ans2 = sum[i];
            d_w_mi = d_w[i];
        } else if (sum[i] == sum_ma and d_w[i] > d_w_mi) {
            sum_ma = sum[i];
            ans1 = i;
            ans2 = sum[i];
            d_w_mi = d_w[i];
